      ...
      : 
        <>
          ...
          {[...posts].reverse().map((post, index) => (
            <TextCard loc={post.location} date={post.postDate} description={post.desc} name={post.uName} type={post.postType} first={index === posts.length - 1} image={post.image ? post.image : null} stickers={post?.stickers} isNarrowScreen={isNarrowScreen}/>
          ))}
        </>
      }
    </div>
